...
class BoardSquare extends Component {
	...
	componentDidMount() {
			this.setState({
				...
				curClasses: `white ${this.props.background}-${this.props.figureColor}`
			});
		this.props.lastmove && this.setState({  lastmove: 'last-move' });
		this.props.lighting === true && this.setState({ lighting: 'circle' });
		const img = new Image();
	...
	componentDidUpdate(prevProps, prevState, multi) {
		if (prevProps !== this.props) {
			this.setState({ curClasses: `white ${this.props.background}-${this.props.figureColor}` });
			this.props.lastmove && this.props.animate !== true && this.animation !== true && this.setState({ lastmove: 'last-move' });
			this.props.lighting === true && this.setState({ lighting: 'circle' });
			this.props.lighting !== true && this.setState({ lighting: '' });
			!this.props.lastmove && this.setState({ lastmove: '' });
			if (this.props.animate !== true && this.animation !== true) {
				this.setState({
					position: this.props.position
				});
			}	else if (this.props.animate === true && this.props.lastmove)  {
				this.final = this.props.lastmove[0];
				this.start = this.props.lastmove[1];

				console.log(this.start, this.state.position, this.props.lastmove, this.props.animate);
				this.props.mult === 700 ? multi = -1 : multi = 1;
				this.ElY = multi*(this.final[0] - this.start[0])*10;
				this.ElX = multi*(this.final[1] - this.start[1])*10;
				this.setState({
					position: [Math.abs(this.props.mult-this.start[1]*100), Math.abs(this.props.mult-this.start[0]*100)]
				});
				this.animation = true
				this.framing = setInterval(this.frame.bind(this), 10);
			 }
		}
	}

	animate(start, final) {
		this.framing = setInterval(this.frame, 5);
	}

	frame() {
		var posX = this.final[0];
		var posY = this.final[1];
		console.log(this.final, this.state.position);
		if (this.state.position[0] === Math.abs(this.props.mult-this.final[1]*100) && this.state.position[1] === Math.abs(this.props.mult - this.final[0]*100)) {
			console.log('???');
			this.animation = false;
			this.setState({ lastmove: 'last-move' });
			clearInterval(this.framing);
			return
		}
		posX = this.state.position[0] + this.ElX;
		posY = this.state.position[1] + this.ElY;

		this.setState({position: [posX, posY]})
	}

	render() {
		const { connectDragSource, connectDropTarget } = this.props;
		var light;
		/*var newEl = this.props.animate === true ? `translate(${this.state.position[0]}px, ${this.state.position[1]}px)` :`translate(${this.props.position[0]}px, ${this.props.position[1]}px)`*/
		/*var newEl = this.props.animate === true ? `translate(${this.props.position[0] + 50}px, ${this.props.position[1] + 50}px)` :`translate(${this.props.position[0]}px, ${this.props.position[1]}px)`*/

		var lastmove = '';

		return compose(connectDragSource, connectDropTarget)(
			<div
				style={{transform: `translate(${this.state.position[0]}px, ${this.state.position[1]}px)`}}
				className={`${this.state.curClasses} ${this.state.lastmove} ${this.state.lighting}`}
...
export default compose(
	DragSource(ItemTypes.KNIGHT, squareTarget, connect => ({
		connectDragSource: connect.dragSource(),
		connectDragPreview: connect.dragPreview()
	})),
	DropTarget(ItemTypes.KNIGHT, noteTarget, connect => ({
